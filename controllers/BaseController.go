...
// sortBySummary 在markdown头部加上<bookstack></bookstack>或者<bookstack/>，即解析markdown中的ul>li>a链接作为目录
func (this *BaseController) sortBySummary(bookIdentify, htmlStr string, bookId int) string {
	...
	o := orm.NewOrm()
	qs := o.QueryTable("documents").Filter("book_id", bookId)
...
func (this *BaseController) replaceLinks(bookIdentify string, docHtml string, isSummary ...bool) string {
	...
	o.QueryTable("books").Filter("identify", bookIdentify).One(&book, "book_id")
	if book.BookId > 0 {
		o.QueryTable("documents").Filter("book_id", book.BookId).Limit(5000).All(&docs, "identify", "document_id")
		if len(docs) > 0 {
