...
$(function () {
    ...
    $(".change-update").change(function () {
        var _this = $(this), _url = _this.attr("data-url"), field = _this.attr("name"),
            method = _this.attr("data-method"), refresh = _this.attr("refresh") == "true";
    ...
    // 更换友链的图标
    $(".link-manager .pic-link-small").click(function () {
        var form = $("form.link-pic-form");
        form.find("[name=id]").val($(this).attr("data-id"));
        form.find("[name=pic]").trigger("click");
    })
    $("#link-pic").change(function () {
        var _this = $(this), action = _this.parents("form").attr("action");
        if (_this.val() != "") {
            var formData = new FormData();
            var id = _this.parents("form").find("[name=id]").val()
            formData.append('pic', _this.get(0).files[0])
            formData.append('id', id)
            upload(action, formData, function (res) {
                if (res.errcode == 0) { // 成功
                    alertTips('success', res.message, 3000, "");
                    $("[data-id=" + id + "]").attr("src", res.data.pic)
                } else { // 失败
                    alertTips('error', res.message, 3000, "")
                }
                _this.val('')
