...
func (m *MigrationVersion03) MigrationNewTableData() error {
	...
	_, err := o.Raw("UPDATE members SET auth_method = 'local'").Exec()
	if err != nil {
		return err
	}
	_, err = o.Raw("INSERT INTO options (option_title, option_name, option_value) SELECT '版本控制','ENABLE_DOCUMENT_HISTORY','true' WHERE NOT exists(SELECT * FROM options WHERE option_name = 'ENABLE_DOCUMENT_HISTORY');").Exec()
	if err != nil {
		return err
	}
	return nil
...
func (m *MigrationVersion03) RollbackMigration() error {
	...
	o := orm.NewOrm()
	_, err := o.Raw("ALTER TABLE members DROP COLUMN auth_method").Exec()
	if err != nil {
		return err
	}

	_, err = o.Raw("DROP TABLE document_history").Exec()
	if err != nil {
		return err
	}
	_, err = o.Raw("DELETE options WHERE option_name = 'ENABLE_DOCUMENT_HISTORY'").Exec()
