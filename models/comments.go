...
func (this *Comments) Comments(p, listRows, bookId int, status ...int) (comments []BookCommentsResult, err error) {
	sql := `select c.id,c.content,s.score,c.uid,c.status,c.time_create,m.avatar,m.nickname,m.account,b.book_id,b.book_name,b.identify from comments c left join members m on m.member_id=c.uid left join score s on s.uid=c.uid and s.book_id=c.book_id left join books b on b.book_id = c.book_id %v order by c.id desc limit %v offset %v`
	whereStr := ""
...
func (this *Comments) ClearComments(uid int) {
	...
	o := orm.NewOrm()
	sql := "select count(id) cnt,id,book_id from comments where uid = ? group by book_id"
	...
	if len(comments) > 0 {
		...
		sqlUpdate := "update books set cnt_comment = cnt_comment - ? where cnt_comment> ? and book_id = ?"
...
func (this *Comments) DeleteComment(id int) {
	...
	if m.BookId > 0 {
		...
		sql := "update books set cnt_comment = cnt_comment - 1 where cnt_comment > 0 and book_id = ?"
...
func (this *Score) BookScores(p, listRows, bookId int) (scores []BookScoresResult, err error) {
	sql := `select s.score,s.time_create,m.avatar,m.nickname from score s left join members m on m.member_id=s.uid where s.book_id=? order by s.id desc limit %v offset %v`
	sql = fmt.Sprintf(sql, listRows, (p-1)*listRows)
	_, err = orm.NewOrm().Raw(sql, bookId).QueryRows(&scores)
	return
}

// BookScoreByUid 查询用户对文档的评分
func (this *Score) BookScoreByUid(uid, bookId interface{}) int {
	var score Score
	orm.NewOrm().QueryTable("score").Filter("uid", uid).Filter("book_id", bookId).One(&score, "score")
	return score.Score
}
...
func (this *Comments) AddComments(uid, bookId int, content string) (err error) {
	...
	now := time.Now()
	o := orm.NewOrm()
	o.QueryTable("comments").Filter("uid", uid).Filter("TimeCreate__gt", now.Add(-time.Duration(second)*time.Second)).OrderBy("-Id").One(&comment, "Id")
	if comment.Id > 0 {
	...
	// 书籍被评论数量量+1
	SetIncreAndDecre("books", "cnt_comment", fmt.Sprintf("book_id=%v", bookId), true)
	return
}
