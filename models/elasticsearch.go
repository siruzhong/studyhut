...
type ElasticSearchClient struct {
	...
	IsRelateSearch bool          // 是否关联查询
}

// ElasticSearchData 全文搜索数据
...
// ElasticSearchCount 统计信息结构
type ElasticSearchCount struct {
	Shards struct { // 分片信息
		Failed     int `json:"failed"`     // 失败的
		Skipped    int `json:"skipped"`    // 跳过的
		Successful int `json:"successful"` // 成功的
		Total      int `json:"total"`      // 总的
	} `json:"_shards"`
...
// NewElasticSearchClient 创建全文搜索客户端
...
// html2Text 将HTML转成符合elasticsearch搜索的文本
...
// Init 初始化全文搜索客户端，包括检查索引是否存在，mapping设置等
...
// BuildIndexByBuck 通过bulk，批量创建/更新索引
...
// SetBookPublic 设置书籍的公有和私有，需要根据文档同时更新文档的公有和私有
