...
// GetFansList 查询用户的粉丝（用户id作为被关注对象）
func (this *Fans) GetFansList(uid, page, pageSize int) (fans []FansResult, total int64, err error) {
	...
	if total > 0 {
		sql := fmt.Sprintf(
			"select m.member_id uid,m.avatar,m.account,m.nickname from members m left join fans f on m.member_id=f.fans_id where f.uid=?  order by f.id desc limit %v offset %v",
...
// GetFollowList 查询用户的关注（用户id作为fans_id）
func (this *Fans) GetFollowList(fansId, page, pageSize int) (fans []FansResult, total int64, err error) {
	...
	if total > 0 {
		sql := fmt.Sprintf(
			"select m.member_id uid,m.avatar,m.account,m.nickname from members m left join fans f on m.member_id=f.uid where f.fans_id=?  order by f.id desc limit %v offset %v",
			pageSize, (page-1)*pageSize,
		)
