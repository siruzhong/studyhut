...
// Member 用户
type Member struct {
	MemberId                   int       `orm:"pk;auto;column(member_id)" json:"member_id"`                         // 用户id
	Account                    string    `orm:"size(30);unique;column(account)" json:"account"`                     // 账户名
	Nickname                   string    `orm:"size(30);unique;column(nickname)" json:"nickname"`                   // 用户昵称
	Password                   string    `orm:"column(password);size(512)" json:"-"`                                // 用户密码(不进行序列化)
	AuthMethod                 string    `orm:"column(auth_method);default(local);size(50);" json:"auth_method"`    // 认证方式: local_本地数据库 ldap_LDAP
	Description                string    `orm:"column(description);size(2000)" json:"description"`                  // 用户描述
	Email                      string    `orm:"size(100);column(email);unique" json:"email"`                        // 用户邮箱
	Phone                      string    `orm:"size(255);column(phone);null;default(null)" json:"phone"`            // 用户手机号
	Avatar                     string    `orm:"column(avatar)" json:"avatar"`                                       // 用户头像
	Role                       int       `orm:"column(role);type(int);default(1);index" json:"role"`                // 用户角色: 0_超级管理员 1_管理员 2_普通用户
	RoleName                   string    `orm:"-" json:"role_name"`                                                 // 用户角色名称(不进行orm映射)
	Status                     int       `orm:"column(status);type(int);default(0)" json:"status"`                  // 用户状态：0_正常 1_禁用
	CreateTime                 time.Time `orm:"type(datetime);column(create_time);auto_now_add" json:"create_time"` // 用户创建时间
	CreateAt                   int       `orm:"type(int);column(create_at)" json:"create_at"`                       // 用户创建在
	LastLoginTime              time.Time `orm:"type(datetime);column(last_login_time);null" json:"last_login_time"` // 用户上次登陆时间
	...
	WechatNO                   string    `json:"wechat_no" orm:"column(wechat_no);size(50)"`                        // 用户微信号
...
func (m *Member) Delete(oldId int, adminId int) (err error) {
	...
	_, err = o.Raw("DELETE FROM members WHERE member_id = ?", oldId).Exec()
	if err != nil {
		return
	}

	var books []Book
	o.QueryTable("books").Filter("member_id", oldId).Limit(10000000).All(&books, "book_id")
	...
	if len(books) > 0 {
		...
		_, err = o.Raw("UPDATE books SET member_id = ? WHERE member_id = ?", adminId, oldId).Exec()
		if err != nil {
			return
		}

		_, err = o.Raw("UPDATE document_history SET member_id=? WHERE member_id = ?", adminId, oldId).Exec()
		if err != nil {
			return err
		}

		_, err = o.QueryTable("documents").Filter("book_id__in", booksId...).Update(orm.Params{"member_id": adminId})
		if err != nil {
			return err
		}
	}
...
// GetUsernameByUid 获取用户名
func (m *Member) GetUsernameByUid(id interface{}) string {
	var user Member
	orm.NewOrm().QueryTable("members").Filter("member_id", id).One(&user, "account")
	return user.Account
}

// GetNicknameByUid 获取昵称
...
// GetQrcodeByUid 根据用户id获取二维码
func (m *Member) GetQrcodeByUid(uid interface{}) (qrcode map[string]string) {
	var member Member
	orm.NewOrm().QueryTable("members").Filter("member_id", uid).One(&member, "alipay", "wxpay")
