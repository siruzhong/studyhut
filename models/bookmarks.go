...
// InsertOrDelete 添加或移除书签（如果书签不存在，则添加书签，如果书签存在，则移除书签）
...
func (m *Bookmark) List(uid, bookId int) (bl []bookmarkList, rows int64, err error) {
	...
	sql := "select %v from bookmark b left join documents d on b.doc_id=d.document_id where b.uid=? and b.book_id=? order by b.id desc limit 1000"
