...
func (m *DocumentHistory) Clear(docId int) error {
	o := orm.NewOrm()
	_, err := o.Raw("DELETE from document_history WHERE document_id = ?", docId).Exec()
	if err == nil {
...
FROM document_history AS history
LEFT JOIN members AS m1 ON history.member_id = m1.member_id
LEFT JOIN members AS m2 ON history.modify_at = m2.member_id
