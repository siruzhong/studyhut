...
type Relationship struct {
	RelationshipId int `orm:"pk;auto;unique;column(relationship_id)" json:"relationship_id"` // 关系id
	MemberId       int `orm:"column(member_id);type(int)" json:"member_id"`                  // 成员id
	BookId         int `orm:"column(book_id);type(int);index" json:"book_id"`                // 书籍id
	RoleId         int `orm:"column(role_id);type(int);index" json:"role_id"`                // 角色：0 创始人(创始人不能被移除) / 1 管理员 /2 编辑者 /3 观察者
}
