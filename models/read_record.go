...
// ReadRecord 阅读记录(用于记录阅读的文档，以及阅读进度统计)
...
var (
	tableReadRecord = "read_record"
	tableReadCount  = "read_count"
...
func (this *ReadRecord) HistoryReadBook(uid, page, size int) (books []Book) {
	// 由于books 没有 created_at 这个字段，所以这里将这个字段映射到version里面...
...
func (this *ReadRecord) List(uid, bookId int) (lists []RecordList, cnt int64, err error) {
	...
	sql := "select %v from %v r left join documents d on r.doc_id=d.document_id where r.book_id=? and r.uid=? order by r.id desc limit 5000"
