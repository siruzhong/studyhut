...
func (m *MemberRelationshipResult) FindForUsersByBookId(bookId, pageIndex, pageSize int) ([]*MemberRelationshipResult, int, error) {
	...
	sql1 := "SELECT * FROM relationship AS rel LEFT JOIN members as member ON rel.member_id = member.member_id WHERE rel.book_id = ? ORDER BY rel.relationship_id DESC  LIMIT ?,?"

	sql2 := "SELECT count(*) AS total_count FROM relationship AS rel LEFT JOIN members as member ON rel.member_id = member.member_id WHERE rel.book_id = ?"
