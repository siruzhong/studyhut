...
func (m *DocumentSearchResult) FindToPager(keyword string, pageIndex, pageSize, memberId int) (searchResult []*DocumentSearchResult, totalCount int, err error) {
	...
	if memberId <= 0 {
		sql1 := `SELECT count(doc.document_id) as total_count FROM documents AS doc
  LEFT JOIN books as book ON doc.book_id = book.book_id
WHERE book.privately_owned = 0 AND (doc.document_name LIKE ? OR doc.release LIKE ?) `

		sql2 := `SELECT doc.document_id,doc.modify_time,doc.create_time,doc.document_name,doc.identify,doc.release as description,doc.modify_time,book.identify as book_identify,book.book_name,rel.member_id,member.account AS author FROM documents AS doc
  LEFT JOIN books as book ON doc.book_id = book.book_id
  LEFT JOIN relationship AS rel ON book.book_id = rel.book_id AND rel.role_id = 0
...
WHERE book.privately_owned = 0 AND (doc.document_name LIKE ? OR doc.release LIKE ?)
 ORDER BY doc.document_id DESC LIMIT ?,? `
		...
		sql1 := `SELECT count(doc.document_id) as total_count FROM documents AS doc
  LEFT JOIN books as book ON doc.book_id = book.book_id
  LEFT JOIN relationship AS rel ON doc.book_id = rel.book_id AND rel.role_id = 0
  LEFT JOIN relationship AS rel1 ON doc.book_id = rel1.book_id AND rel1.member_id = ?
...
func (m *DocumentSearchResult) SearchDocument(keyword string, bookId int, page, size int) (docs []*DocumentSearchResult, cnt int, err error) {
	...
	if bookId == 0 {
		...
		sql = "SELECT %v FROM documents d left join books b on d.book_id=b.book_id WHERE b.privately_owned=0 and (d.document_name LIKE ? OR d.`release` LIKE ? )"
...
func (m *DocumentSearchResult) GetDocsById(id []int, withoutCont ...bool) (docs []DocResult, err error) {
	...
	sqlFmt := "select " + strings.Join(fields, ",") + " from documents d left join books b on d.book_id=b.book_id where d.document_id in(%v)"
